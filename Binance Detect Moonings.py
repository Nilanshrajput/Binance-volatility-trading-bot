...
def get_price(add_to_historical=True):
    ...
    for coin in prices:
        ...
        if CUSTOM_LIST:
            if any(item.replace(PAIR_WITH, "") + PAIR_WITH == coin['symbol'] for item in tickers) and all(item not in coin['symbol'] for item in FIATS):
...
def buy_external_signals():
    ...
    signals = glob.glob(f"signals/{sig_type}*.exs")
...
def sell_coins(tpsl_override = False):
    ...
    for coin in list(coins_bought):
        ...
        if time_ch_factor >= 1 and coins_bought[coin]['take_profit']>=MINIMUM_SP and  coins_bought[coin]['ttp_enabled'] == False:
            coins_bought[coin]['take_profit'] = TAKE_PROFIT- time_ch_factor*TP_CHANGE
            #print (f" TP reduced by { txcolors.SELL_PROFIT} {time_ch_factor*TP_CHANGE } now it is { coins_bought[coin]['take_profit']}")
            #print(((datetime.now().timestamp() - coins_bought[coin]['timestamp'])))

        #print(coins_bought[coin]['take_profit'])
        ...
        if LastPriceLessFees > TP and USE_TRAILING_STOP_LOSS and not sell_all_coins and not tpsl_override or ( PriceChange_Perc > coins_bought[coin]['take_profit'] and coins_bought[coin]['take_profit']<-0):
            ...
            if PriceChangeIncFees_Perc >= 0.8:
                ...
                coins_bought[coin]['stop_loss'] = coins_bought[coin]['take_profit'] - TRAILING_STOP_LOSS
                #coins_bought[coin]['take_profit'] = PriceChange_Perc + TRAILING_TAKE_PROFIT
                if not coins_bought[coin]['take_profit']<TAKE_PROFIT:
                    coins_bought[coin]['take_profit'] = PriceChangeIncFees_Perc + TRAILING_TAKE_PROFIT
                else:
                    coins_bought[coin]['take_profit'] = PriceChange_Perc + TRAILING_TAKE_PROFIT
...
if __name__ == '__main__':
    ...
    TP_CHANGE = parsed_config['trading_options']['TP_CHANGE']
    MINIMUM_SP = parsed_config['trading_options']['MINIMUM_SP']
    TP_CHANGE_TIME = parsed_config['trading_options']['TP_CHANGE_TIME']
    ...
    USE_TRAILING_BUY= parsed_config['trading_options']['USE_TRAILING_BUY']
